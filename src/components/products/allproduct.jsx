...
//   useEffect(() => {
//     const total = Math.ceil(toalProduct / ResultPerPage);
//     setTotalPages(total);
//   }, [toalProduct, ResultPerPage]);

//   useEffect(() => {
//     fetchWishlist(setWishlistItems);
...
//                 <img src='https://minteventrentals.com/public/templates/mint/images/noproductfound.png' style={{ maxWidth: "800px" }} />
//               </div>
//             ) : (
...
//                                 <LazyLoadImage effect="blur" loading="lazy" src={RemoveIcon} alt="RemoveIcon" className='Icon_add_to_cart' onClick={() => removeFromCart(product?._id, setProductLoaders, setCartItems, fetchCart)} />
...
function Allproduct({ search, category, minPrice, maxPrice }) {
  ...
  const fetchProduct = async () => {
    try {
      setAllProductLoader(true);
      const response = await makeApi(`/api/get-all-products?name=${search}&category=${category}&minPrice=${minPrice}&maxPrice=${maxPrice}&productType=${productType}&IsOutOfStock=false`, "GET");
      setProducts(response.data.products);
    } catch (error) {
      console.log(error);
    } finally {
